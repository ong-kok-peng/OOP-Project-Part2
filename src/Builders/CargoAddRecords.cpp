...
void CargoAddRecords::addRecordsFromFile() {
	...
	if (!readFile.readFile(userInput)) {
		//error in opening the file or no lines were read
		cout << "\"" << userInput << "\" cannot be read. File path is invalid or empty file. Press enter to go back.\n";
	}
	else {
		...
		int lineNumber = 0;
		while (lineNumber < readFile.getLineCount()) {
			string fileLine = readFile.getFileLine(lineNumber);

			stringstream ss(fileLine);
			if (!(getline(ss, id, ',') && getline(ss, destination, ',')
				&& getline(ss, time, ',') && getline(ss, quantityStr))) {
				//if line has < 4 parameters
				cout << "Line " << (lineNumber + 1) << ": Blank input or cargo ID/destination/time/quantity is/are missing.\n";
